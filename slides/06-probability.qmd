...
#| echo: false
#| out-width: 75%
knitr::include_graphics("./images/06-probability/chance_09_14_2026.png") 
```
:::
...
```{r}
#| out-width: 100%
knitr::include_graphics("./images/06-probability/coin_toss.png")
```
:::
...
::: tip
Draw a [**Venn Diagram**](https://chenghanyustats-introstat-venndiagram.share.connect.posit.cloud) every time you get stuck!
:::
...
```{r}
#| out-width: 60%
knitr::include_graphics("./images/06-probability/test_table_new.png")
```
:::
...
$$\begin{align*} P(A \mid B) &= \frac{P(A \cap B)}{P(B)} \quad ( \text{cond. prob.}) \\ &= \frac{P(A \cap B)}{P((B \cap A) \cup (B \cap A^c))} \quad ( \text{partition } B) \\ &= \frac{P(B \mid A)P(A)}{P(B \mid A)P(A) + P(B \mid A^c)P(A^c)}  \quad ( \text{multiplication rule}) \end{align*}$$
