...
plot(c("0", "1", "2"), c(0.25, 0.5, 0.25), type = "h", ylim = c(0, 0.75),
     lwd = 10, col = "#003366", ylab = "P(X = x)", xlab = "x", 
...
<!-- ## [Binomial Distribution Applet](https://math4720-f26.github.io/website/ojs/prob-binomial-ojs.html) -->
...
plot(x = 0:15, y = dbinom(0:15, size = 15, prob = 0.2), type = 'h', xlab = "x", 
     ylab = "P(X = x)", lwd = 10, main = "Binomial(15, 0.2)", bty = "none")
```
...
::: tip
Never hand calculate this. We compute them using R!
:::
...
```{webr}
## Compute P(X >= 8)
pbinom(q = ____, size = _____, prob = _____, lower.tail = _____)
```
...
par(mar = c(4, 4, 2, 0), mgp = c(2, 0.5, 0), las = 1)
plot(x = 0:15, y = dbinom(0:15, size = 15, prob = 0.2), type = 'h', xlab = "x", 
     ylab = "P(X = x)", lwd = 10, main = "Binomial(15, 0.2)")
```
...
- Let $X$ be a Poisson r.v. Then $\color{blue}{X \sim Poisson(\lambda)}$, where $\lambda$ is the parameter representing the **mean** number of occurrences of the event in the space/time interval.
...
<!-- ## [Poisson Distribution Applet](https://math4720-f26.github.io/website/ojs/prob-poisson-ojs.html) -->
...
1. $\small \lambda = \dfrac{\text{Number of birth in a year}}{\text{Number of days}} = \dfrac{4200}{365} = 11.5$

. . .

2. $\small P(X = 10 \mid \lambda = 11.5) = \dfrac{\lambda^x e^{-\lambda}}{x!} = \dfrac{11.5^{10} e^{-11.5}}{10!} = 0.113$
...
```{webr}
## Compute P(X >= 8)
ppois(q = ____, lam = _____, lower.tail = _____)
```
...
plot(0:24, dpois(0:24, lambda = 4200 / 365), type = 'h', lwd = 10, las = 1,
...
- A continuous r.v. $X$ has a **probability density function (pdf)** $f(x)$ such that for any real value $a < b$,
...
- Every pdf must satisfy <span style="color:blue"> (1) $f(x) \ge 0$ for any possible $x$; (2) $\int_{-\infty}^{\infty} f(x) dx = 1$ </span>
...
y = dgamma(x, 2, 1/2)
plot(x, y, type = "l", lwd = 4, col = "blue", axes = FALSE, 
     ylab = "f(x)", las = 1, cex.lab = 1.4)
axis(1, at = c(2, 4), labels = c("a", "b"), tick = TRUE, cex.lab = 2)
...
text(7, dgamma(5.6, 2, 1/2), "density curve", cex = 1.8)
```
...
- [Distribution Applet](https://chenghanyustats-distributions.share.connect.posit.cloud/)
...
-  The normal distribution, $N(\mu, \sigma^2$), has the pdf:
...
```{r}
par(mfrow = c(1, 1))
par(mar = c(1.8, 0, 2, 0), mgp = c(0.5, 0.5, 0), las = 1)
mean=100; sd=15
# lb=80; ub=120
x <- seq(-4,4,length=100)*sd + mean
hx <- dnorm(x,mean,sd)
plot(x, hx, type="n", xlab="x", ylab="", cex.lab = 1.5,
  main=expression(N(100, 15^2)), axes=FALSE, cex.main = 2)
# i <- x >= lb & x <= ub
...
# mtext(result,3)
axis(1, at=seq(40, 160, 20), pos=0, tick = -0.005, cex.axis = 1.6)
```
...
<!-- ## [Normal Distribution Applet](https://math4720-f26.github.io/website/ojs/prob-normal-ojs.html) -->

<!-- {{< include prob-normal-ojs.qmd >}} -->

## Standardization and Z-Scores

- **Standardization**: Convert $N(\mu, \sigma^2)$ into $N(0, 1)$.
...
plot(x, hz, type="n", xlab="x", ylab="", ylim = c(0, dnorm(0, 0, 1)),
  main = "Shift from N(3, 4) to N(0, 4)", axes = FALSE, cex.main = 1.6,
  cex.lab = 1.5)
lines(x, hz, col = "blue", lwd = 4)
lines(x, hx, col = "#003366", lwd = 4)
axis(1, at = seq(-3, 9, 1), pos=0, cex.axis = 1.5, tck = -0.01)
# abline(v = c(0, 3))
...
hz <- dnorm(x, 0, 1)
plot(x, hz, type="n", xlab="x", ylab="", ylim = c(0, dnorm(0, 0, 1)),
  main = "Scale from N(0, 4) to N(0, 1)", axes = FALSE, cex.main = 1.6,
  cex.lab = 1.5)
...
plot(X, Y,
     type = 'l',
     axes = FALSE, xlab = "",
     xlim = m + s * 2.7 * c(-1, 1), lwd = 2)
axis(1, at = m + s * (-3:3), cex.axis = 1.5)
abline(h = 0)
lines(c(m, m),
...
normTail(m = 1100, s = 200, L = 1300, col = 4, cex.axis = 1.5, lwd = 2)
```
...
normTail(m = 1100, s = 200, U = 1190, col = 4, cex.axis = 1.5, lwd = 2)
```
...
normTail(m = 1100, s = 200, L = qnorm(0.95, 1100, 200), col = 4, cex.axis = 1.5, lwd = 2)
