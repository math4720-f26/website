...
- As of July 2026, there are about 24186 R packages available on [**CRAN**](https://cran.r-project.org/) (the Comprehensive R Archive Network)!
...
#| out-width: 75%
#| fig-align: center
knitr::include_graphics("./images/05-data-description/skew_gpt.png")
```
...
<!-- ## [Outliers Applet](https://math4720-f26.github.io/website/ojs/data-extremes-ojs.html) -->
...
- **p-th percentile (quantile)** ($P_p$): a data value such that 
  + at least $p\%$ of the values are $\leq P_p$
  + at least $(1-p)\%$ of the values are are $\geq P_p$
...
#| echo: false
#| out-width: 100%
knitr::include_graphics("./images/05-data-description/boxplot_no_outlier.png")
```
...
#| echo: false
#| out-width: 100%
knitr::include_graphics("./images/05-data-description/boxplot_outlier.png")
```
...
`boxplot()` uses Tukey hinges; `quantile()` uses a default interpolation rule. They agree for odd sample sizes, but can differ for even sample sizes. Use `boxplot(body_mass)$stats` to check hinges values.
...
boxplot() uses Tukey’s hinges via fivenum(), not quantile.

The two ‘hinges’ are versions of the first and third quartile, i.e., close to quantile(x, c(1,3)/4). The hinges equal the quartiles for odd 
n
n (where n <- length(x)) and differ for even 
n
n. Whereas the quartiles only equal observations for n %% 4 == 1 (
n
≡
1
m
...
d
4
n≡1mod4), the hinges do so additionally for n %% 4 == 2 (
n
≡
2
m
...
d
4
n≡2mod4), and are in the middle of two observations otherwise

要讓 quantile() 模仿 base R boxplot() 的 Tukey hinges，需要依照樣本數 n 切換：
